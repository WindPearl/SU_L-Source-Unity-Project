...
public abstract class Player : MonoBehaviour, IDamageable
{
    ...
    public bool useBumpersToSwitchRails;
    ...
    public float slideVelocityTransfer;
    ...
    public bool boostDepletesRings = false;
    ...
    public virtual void Update()
    {
        ...
        if (isBoosting && boostDepletesRings == true)
        {
        ...
        if (ringEnergy <= 0 && boostDepletesRings == true)
        {
            isBoosting = false;
        }

        driftStartDirection = absoluteLeftStick;

        //Debug.Log(rigidbody.velocity.magnitude);
        // Debug.Log("absoluteVelocity" + absoluteVelocity);
        ...
        {
            rawDirection = Input.GetAxisRaw(XboxAxis.LeftStickX);
        }
        else if (Input.GetAxisRaw(XboxAxis.LeftStickX) < 0)
        {
            rawDirection = Input.GetAxisRaw(XboxAxis.LeftStickX);
        }
   //     Debug.Log( Input.GetAxisRaw(XboxAxis.LeftStickX));
    ...
    public void CheckBoost()
    {
        ...
        else if (boostDepletesRings == false)
        {
    ...
    public void StateMove3D()
    {
        ...
        if (Input.GetButtonDown(XboxButton.X) && boostDepletesRings == true)
        {
        ...
        if (Input.GetButtonDown(XboxButton.X) && boostDepletesRings == false)
        {
            if (isBoosting == false)
            {
                SendMessage("StateBoostStart");
    ...
    {
        slideVelocity = rigidbody.velocity.magnitude;
        slideVelocityTransfer = absoluteVelocity;
    }
    ...
    public void StateSlidingPhysics()
    {
       ...
       if (slideTimer > 1.5) 
        {
          //  rigidbody.velocity = transform.forward * (slideVelocity / (rigidbody.velocity.magnitude / (absoluteVelocity / 2)));
            rigidbody.AddForce(leftStickDirection * 10, ForceMode.Acceleration);
            //   slideVelocity = rigidbody.velocity - 1;
            //   rigidbody.velocity = transform.forward * (slideVelocity * (rigidbody.velocity.magnitude));
        }
       /* else if(slideTimer > 3)
        {
            slideVelocity = (slideVelocity / (absoluteVelocity)) / 2;
        }*/
       else
        {
            Vector3 direction = Vector3.Cross(transform.right, GetGroundInformation().normal);
            rigidbody.velocity = direction * rigidbody.velocity.magnitude;
            rigidbody.velocity *= (slideVelocity / (absoluteVelocity));
        //    rigidbody.velocity = rigidbody.velocity * 2;
        }
      //  Debug.Log("Sliding Velocity " + transform.forward * (slideVelocity / (rigidbody.velocity.magnitude / absoluteVelocity)));
    ...
    private void StateSlidingEnd()
    {
        ...
        slideVelocity = 0;
        driftVelocity = 0;
    //   absoluteVelocity = slideVelocityTransfer * 2;
    ...
    public void StateGrind()
    {
        ...
        {
            if ((grindSplineSensorL.bezierPath && Input.GetAxis(XboxAxis.LeftStickX) < -deadZone) && useBumpersToSwitchRails == false)
            {
            ...
            else if ((grindSplineSensorR.bezierPath && Input.GetAxisRaw(XboxAxis.LeftStickX) > deadZone) && useBumpersToSwitchRails == false)
            {
            ...
            else if ((grindSplineSensorR.bezierPath && Input.GetButton(XboxButton.RightShoulder)) && useBumpersToSwitchRails == true)
            {
                tempSensor.bezierPath = grindSplineSensorR.bezierPath;
            ...
            else if ((grindSplineSensorL.bezierPath && Input.GetButton(XboxButton.LeftShoulder)) && useBumpersToSwitchRails == true)
            {
                tempSensor.bezierPath = grindSplineSensorL.bezierPath;
    ...
    private void StateDriftStart()
    {
       // driftStartDirection = rawDirection;
        driftVelocity = rigidbody.velocity.magnitude;
    }
    ...
    private void StateDrift()
    {
       ...
       driftStartDirection = -rawDirection;
        ...
        rigidbody.velocity = transform.forward * (driftVelocity / (rigidbody.velocity.magnitude / absoluteVelocity));
        ...
        Vector3 direction = Vector3.Cross(transform.right, GetGroundInformation().normal);
        rigidbody.velocity = direction * rigidbody.velocity.magnitude;
      //  Debug.Log(rigidbody.velocity = direction * rigidbody.velocity.magnitude);
      //  Debug.Log("Drift Velocity" + driftVelocity);
    ...
    private void StateDriftEnd()
    {
        ...
        absoluteVelocity = driftVelocity * rigidbody.velocity.magnitude;
   //     Debug.Log("absoluteVelocity" + absoluteVelocity);
        driftVelocity = 0;
        slideVelocity = 0;
    }
