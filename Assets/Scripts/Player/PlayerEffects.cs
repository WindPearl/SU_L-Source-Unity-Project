...
public class PlayerEffects : MonoBehaviour
{
    ...
    public ParticleSystem waterRun2;
    ...
    private void Update()
    {
        ...
        if (player.IsGrounded() && !player.underwater)
        {
            ...
            else if (player.GetGroundInformation().collider.sharedMaterial && player.GetGroundInformation().collider.sharedMaterial.name == "Water")
            { 
                if (waterRun.isStopped)
                {
                    waterRun.Play();
